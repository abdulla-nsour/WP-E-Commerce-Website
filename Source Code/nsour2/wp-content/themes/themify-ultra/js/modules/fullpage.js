	...
	const _is_retina = window.devicePixelRatio > 1,
			...
			run = (w)=> {
                ...
                if(!duration){
					...
					duration = parseFloat(comp.transitionDuration);
					if (duration < 100) {
						duration *= 1000;
					}
				}
			...
			_scrollVertical = (horizontalIndex, dir,silient)=> {
				if(scrolling===false){
						...
						if (next) {
							...
							if (next) {
								let tr='none',
                                    trEnd=function (e) {
										if(e.target===this){
											this.removeEventListener('transitionend', trEnd, {passive: true});
											this.style['transition'] = this.style['transform'] ='';
										}
									};
									next.addEventListener('transitionend', trEnd, {passive: true});
									if(dir==='prev'){
					...
					if (silient === true) {
						...
						wrapper.addEventListener('transitionend', trEnd, {passive: true});
					}
			...
			_scrollHorizontally = (container,dir, silient)=> {
				if(scrolling===false){
					...
					if (silient === true) {
						...
						inner.addEventListener('transitionend',trEnd, {passive: true});
					}
